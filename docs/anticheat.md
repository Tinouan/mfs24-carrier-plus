# MFS Carrier+ - Systeme Anti-Triche
...
Le systeme anti-triche de MFS Carrier+ protege l'integrite des donnees de jeu en utilisant **SQLite local comme source de verite**. En mode multi, le HOST valide toutes les actions.
...
if (simFuelCurrent > dbFuelGallons + 1) {
  console.warn(`[CarrierPlus] ANTI-CHEAT: Sim fuel (${simFuelCurrent}) > DB fuel (${dbFuelGallons}). Resetting.`);
}
```
...
```
[CarrierPlus] ANTI-CHEAT: Sim fuel (XX.X) > DB fuel (YY.Y). Resetting.
[CarrierPlus] ANTI-CHEAT Hangar: Sim fuel (XX.X) > DB fuel (YY.Y). Resetting.
```
...
| Fichier | Role |
|---------|------|
| `CarrierPlus.tsx` | setSimulatorFuel(), anti-cheat, background tracking |
...
3. Ouvre l'app Carrier+ ou charge une mission

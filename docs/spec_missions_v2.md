...
### Principe cle
Le joueur cree son plan de vol dans MSFS (EFB natif, LittleNavMap, SimBrief, etc.), puis Carrier+ track la progression **sans dependre des SimVars specifiques** qui ne fonctionnent pas avec certains avioniques (Garmin WT notamment).
...
function trackingLoop() {
    ...
    if (this.lastNextWpLat !== 0) {
        ...
        if (wpChange > 1.0) {  // Plus de 1nm de difference = nouveau waypoint
            ...
            console.log(`[CarrierPlus] Waypoint passed! Count: ${this.waypointChangeCount}`);
        }
    }

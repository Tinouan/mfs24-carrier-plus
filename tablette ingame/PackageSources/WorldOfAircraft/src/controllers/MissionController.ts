...
export class MissionController {
  ...
  public async refreshMissionOrigin(): Promise<void> {
    ...
    // DB is the source of truth for display — no SimVar here
    const user = authState.currentUser.get();
    const dbAirport = user?.current_airport || user?.preferred_airport;

    if (dbAirport) {
      console.log(`[WOA] refreshMissionOrigin: using DB current_airport = ${dbAirport}`);
      missionState.missionOriginIcao.set(dbAirport);
      void this.loadCurrentAircraftForMission();
      this.fetchActiveMission();
    } else {
      console.warn("[WOA] refreshMissionOrigin: no current_airport in DB");
    }
  }
  ...
  public async updateCreationSteps(): Promise<void> {
    ...
    // Reference: DB player.current_airport
    const user = authState.currentUser.get();
    const refAirport = user?.current_airport?.toUpperCase() || "";
    ...
    console.log("[WOA] updateCreationSteps:", { step1: step1Valid, locationValid, simAirport, acAirport, refAirport });
  }
  ...
  public async createMissionV11(): Promise<void> {
    ...
    if (typeof SimVar !== "undefined") {
      try {
        ...
        if (simAirport && /^[A-Z]{3,4}$/.test(simAirport) && simAirport !== origin) {
          console.log(`[WOA] Anti-cheat: SimVar airport=${simAirport}, expected=${origin}`);
          missionState.missionError.set(`${this.t("missions", "mustBeAtAirport")} ${origin}`);
          missionState.missionStatus.set("error");
          return;
        }
      } catch {
        console.log("[WOA] Could not read GPS CLOSEST AIRPORT ID SimVar");
      }
    }
    ...
    if (currentAircraft?.current_airport_ident) {
      ...
      if (acLocation !== origin.toUpperCase()) {
        console.log(`[WOA] Anti-cheat: aircraft location=${acLocation}, expected=${origin}`);
    ...
    if (selectedAircraft?.registration && typeof SimVar !== "undefined") {
      try {
        ...
        if (simReg && simReg !== expectedReg) {
          console.log(`[WOA] Anti-cheat: SimVar ATC ID=${simReg}, expected=${expectedReg}`);
        ...
        console.log("[WOA] Could not read ATC ID SimVar");
      }
    }
  ...
  public openCargoPopup(
    ...
    if (maxQty <= 0) {
      if (direction === "unload") {
        console.warn(`[WOA] No ${itemId} in aircraft to unload`);
  ...
  public async loadCurrentAircraftForMission(): Promise<void> {
    ...
    try {
      ...
      if (typeof SimVar !== "undefined") {
        try {
          ...
          if (simAirport && /^[A-Z]{3,4}$/.test(simAirport) && playerAirport && simAirport !== playerAirport) {
            ...
            console.warn(`[WOA] Warning: SimVar airport=${simAirport}, DB airport=${playerAirport}`);
          }
      ...
      if (acDbLocation && playerAirport && acDbLocation !== playerAirport) {
        ...
        console.warn(`[WOA] Warning: aircraft ${acType} at ${acDbLocation}, player at ${playerAirport}`);
      }

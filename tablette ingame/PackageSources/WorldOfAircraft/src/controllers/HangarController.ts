  ...
  public async autoSyncCurrentAircraft(): Promise<void> {
    ...
    try {
      ...
      if (!currentAirport) {
        ...
        if (lat !== 0 || lon !== 0) {
          ...
          if (airport) {
            ...
            console.log(`[WOA] Auto-sync: coordinate fallback detected airport ${currentAirport}`);
          }
        }
        if (!currentAirport) {
          return;
      ...
      if (simFuelCapacity > 0 && dbFuelGallons > 0) {
        ...
        console.warn(`[WOA] DB fuel is 0 or not set for ${matchingAircraft.registration}, NOT resetting simulator fuel`);
      }

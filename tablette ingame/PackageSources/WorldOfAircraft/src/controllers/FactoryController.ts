...
export class FactoryController {
  ...
  async renderFactoryList(): Promise<void> {
    ...
    for (const factory of myFactories) {
      ...
      if (factory.status === "producing" && activeBatch && recipe) {
  ...
  async showFactoryDetail(factoryId: string): Promise<void> {
    ...
    if (factory.status === "idle" && nextTierConfig) {
      html += `<button class="factory-upgrade-btn" style="padding: 6px 10px; background: #8b5cf6; border: none; border-radius: 6px; color: white; font-size: 10px; cursor: pointer;">Upgrade T${factory.tier}->T${factory.tier + 1} (${nextTierConfig.upgrade_cost.toLocaleString()} CR)</button>`;
    }
    ...
    if (assignedWorkers.length === 0) {
      html += `<div style="color: #6b7280; font-size: 10px; padding: 6px; text-align: center;">Aucun worker assigne</div>
        <div style="display: flex; justify-content: space-between; font-size: 10px; color: #6b7280; margin-top: 4px;">
          <span>Bonus workers : +0%</span><span>Cout/h : 0 CR/h</span>
        </div>
        <div style="display: flex; align-items: center; gap: 4px; margin-top: 6px; padding: 4px 6px; background: rgba(59,130,246,0.1); border-radius: 4px;">
          <span style="display:inline-block;width:14px;height:14px;border-radius:50%;background:#3b82f6;color:#fff;text-align:center;font-size:10px;line-height:14px;">i</span>
      ...
      for (const w of assignedWorkers) {
        html += `<div style="display: flex; justify-content: space-between; align-items: center; padding: 6px; background: #0d0d14; border-radius: 4px; margin-bottom: 4px;">
          <div style="display: flex; gap: 6px; align-items: center;">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#f59e0b;"></span>
            <span style="font-size: 10px; color: #60a5fa; font-weight: 600;">${w.country_code}</span>
            <span style="font-size: 10px; color: #9ca3af;">T${w.tier} ${w.hourly_salary} CR/h</span>
          </div>
          <button class="factory-unassign-btn" data-worker-id="${w.id}" style="padding: 2px 6px; background: #374151; border: none; border-radius: 4px; color: #9ca3af; font-size: 9px; cursor: pointer;">Retirer</button>
        </div>`;
      }
      html += `<div style="display: flex; justify-content: space-between; font-size: 10px; color: #9ca3af; margin-top: 4px;">
        <span>Bonus workers : +${workerTierBonusPct}% (tier moy. ${workerTierAvg})</span>
        <span>Cout/h : ${Math.round(workerSalaryCost)} CR/h</span>
      </div>`;
    }
    html += `</div>`;
    ...
    html += `<div style="background: #1a1a24; border-radius: 8px; padding: 10px; margin-bottom: 10px;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        ...
        ${factory.max_engineers > 0
          ...
          : `<span style="font-size: 10px; color: #6b7280;">Disponible a partir de T2</span>`
        }
      </div>`;

    if (assignedEngineers.length === 0) {
      html += `<div style="color: #6b7280; font-size: 10px; padding: 6px; text-align: center;">Aucun ingenieur assigne</div>
        <div style="display: flex; justify-content: space-between; font-size: 10px; color: #6b7280; margin-top: 4px;">
          <span>Bonus ingenieurs : +0%</span><span>Cout/h : 0 CR/h</span>
        </div>`;
      if (factory.max_engineers > 0) {
        html += `<div style="display: flex; align-items: center; gap: 4px; margin-top: 6px; padding: 4px 6px; background: rgba(139,92,246,0.1); border-radius: 4px;">
      ...
      for (const w of assignedEngineers) {
        html += `<div style="display: flex; justify-content: space-between; align-items: center; padding: 6px; background: #0d0d14; border-radius: 4px; margin-bottom: 4px;">
          <div style="display: flex; gap: 6px; align-items: center;">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#8b5cf6;"></span>
            <span style="font-size: 10px; color: #a78bfa; font-weight: 600;">${w.country_code}</span>
            <span style="font-size: 10px; color: #9ca3af;">${w.hourly_salary} CR/h</span>
          </div>
          <button class="factory-unassign-btn" data-worker-id="${w.id}" style="padding: 2px 6px; background: #374151; border: none; border-radius: 4px; color: #9ca3af; font-size: 9px; cursor: pointer;">Retirer</button>
        </div>`;
      }
      html += `<div style="display: flex; justify-content: space-between; font-size: 10px; color: #9ca3af; margin-top: 4px;">
        <span>Bonus ingenieurs : +${engBonusPct}%</span>
        <span>Cout/h : ${Math.round(engSalaryCost)} CR/h</span>
      </div>`;
    }
    html += `</div>`;
    ...
    el.querySelector(".factory-upgrade-btn")?.addEventListener("click", () => void this.handleUpgrade(factoryId));
    ...
    el.querySelectorAll(".factory-unassign-btn").forEach((btn: Element) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const wId = btn.getAttribute("data-worker-id");
        if (wId) void this.handleUnassignWorker(wId);
      });
    });
  ...
  private async showIngredientPicker(factoryId: string, slotIdx: number): Promise<void> {
    ...
    const itemEntries = inventory
      ...
      .map(inv => {
        ...
        return { itemId: inv.item_code, name: item?.name || inv.item_code, item, qty: inv.quantity };
      })
      .sort((a, b) => {
    ...
    let html = `<div style="background:#1a1a24;border-radius:8px;padding:16px;">
      <div style="font-size:14px;font-weight:600;color:white;margin-bottom:12px;">Choisir un ingredient (Slot ${slotIdx + 1})</div>`;
    ...
    if (itemEntries.length === 0) {
      ...
      for (const entry of itemEntries) {
        ...
        html += `<div class="ing-pick-btn" data-item-id="${entry.itemId}" style="display:flex;justify-content:space-between;align-items:center;padding:8px;background:#0d0d14;border-radius:4px;margin-bottom:4px;cursor:${isGrayed ? "default" : "pointer"};border:1px solid transparent;opacity:${isGrayed ? "0.4" : "1"};" ${!isGrayed ? `onmouseover="this.style.borderColor='#3b82f6'" onmouseout="this.style.borderColor='transparent'"` : ""}>
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-size:9px;color:#60a5fa;">${tierLabel}</span>
          </div>
    ...
    html += `<button class="ing-pick-back-btn" style="margin-top:8px;padding:6px 12px;background:#374151;border:none;border-radius:6px;color:white;font-size:10px;cursor:pointer;">Annuler</button>`;
    html += `</div>`;
  ...
  private async showAssignWorkerPopup(factoryId: string): Promise<void> {
    ...
    let html = `<div style="background: #1a1a24; border-radius: 8px; padding: 16px;">
      <div style="font-size: 14px; font-weight: 600; color: white; margin-bottom: 12px;">Assigner un Worker</div>`;
    ...
    html += `<button class="assign-back-btn" style="margin-top: 8px; padding: 6px 12px; background: #374151; border: none; border-radius: 6px; color: white; font-size: 10px; cursor: pointer;">Retour</button>`;
    html += `</div>`;
  ...
  private async showAssignEngineerPopup(factoryId: string): Promise<void> {
    ...
    let html = `<div style="background: #1a1a24; border-radius: 8px; padding: 16px;">
      <div style="font-size: 14px; font-weight: 600; color: white; margin-bottom: 4px;">Assigner un Ingenieur</div>
      <div style="font-size: 10px; color: #9ca3af; margin-bottom: 12px;">${currentCount}/${factory.max_engineers} postes occupes</div>`;
  ...
  private async showDepositFoodPopup(factoryId: string): Promise<void> {
    ...
    let html = `<div style="background: #1a1a24; border-radius: 8px; padding: 16px;">
      <div style="font-size: 14px; font-weight: 600; color: white; margin-bottom: 12px;">Deposer de la nourriture</div>
      <div style="font-size: 10px; color: #9ca3af; margin-bottom: 8px;">Stock: ${factory.food_stock}/${factory.food_capacity}</div>`;
    ...
    if (foodItems.length === 0) {
      ...
      for (const inv of foodItems) {
        const item = ItemService.getItemById(inv.item_code);
        html += `<div class="deposit-food-btn" data-item-id="${inv.item_code}" data-qty="${inv.quantity}" style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: #0d0d14; border-radius: 4px; margin-bottom: 4px; cursor: pointer; border: 1px solid transparent;" onmouseover="this.style.borderColor='#22c55e'" onmouseout="this.style.borderColor='transparent'">
          <div style="display: flex; align-items: center; gap: 8px;">
            ${renderItemIcon(item?.icon_path, item?.icon || "??", item?.tier || 0, 28)}
            <span style="font-size: 11px; color: white;">${item?.name || inv.item_code}</span>
            <span style="font-size: 10px; color: #9ca3af;">x${inv.quantity}</span>
          </div>
          <span style="font-size: 10px; color: #22c55e;">Deposer tout</span>
        </div>`;
      }
    }

    html += `<button class="deposit-back-btn" style="margin-top: 8px; padding: 6px 12px; background: #374151; border: none; border-radius: 6px; color: white; font-size: 10px; cursor: pointer;">Retour</button>`;
    html += `</div>`;

    el.innerHTML = html;

    el.querySelectorAll(".deposit-food-btn").forEach((btn: Element) => {
      btn.addEventListener("click", async () => {
        const itemId = btn.getAttribute("data-item-id");
  ...
  private async handleUpgrade(factoryId: string): Promise<void> {
    try {
      await Services.factories.upgradeFactory(factoryId);
      void this.showFactoryDetail(factoryId);
      void this.renderFactoryList();
    } catch (error: any) {
      console.error("[FactoryController] Upgrade failed:", error.message);
    }
  }

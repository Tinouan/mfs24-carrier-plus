...
class LocalContractServiceClass {
  ...
  async generateAIContracts(playerAirport: string): Promise<void> {
    // Count existing AI offers
    const existing = await DatabaseManager.getAvailableContracts();
    const aiContracts = existing.filter(c => c.creator_type === "ai" && c.status === "available");

    // Generate up to TARGET
  ...
  async refreshContracts(): Promise<void> {
    ...
    // Regenerate AI contracts to maintain ~10 available
    const airport = player.current_airport || "LFPG";

      ...
      <div style={selectedAirport.map(airport => airport
        ...
        <div style="padding: 12px 16px;">
          ...
          <div style="margin-bottom: 8px;">
            <Button style="display: block; width: 100%;" callback={(): void => {
              const airport = selectedAirport.get();
              if (airport) onOpenCreateFactory(airport);
            }}>
              <div style="padding: 12px 0; background: #3b82f6; border-radius: 8px; color: #ffffff; font-size: 13px; font-weight: 600; text-align: center;">
          ...
          <div style={myFactoriesAtAirport.map(factories => factories.length > 0 ? "margin-bottom: 8px;" : "display: none;")}>
            <Button style="display: block; width: 100%;" callback={(): void => {
              const factories = myFactoriesAtAirport.get();
              if (factories.length === 1) {
                onOpenManageFactory(factories[0]);
              } else if (factories.length > 1) {
                onOpenManageFactory({ id: "", name: "" });
              }
            }}>
              <div style="padding: 12px 0; background: #a855f7; border-radius: 8px; color: #ffffff; font-size: 13px; font-weight: 600; text-align: center;">
                {MappedSubject.create(([f, l]) => {
                  if (f.length === 1) return `${translations[l].map.manageFactory} — ${(f[0] as any).name || ""}`;
                  return `${translations[l].map.manageFactory} (${f.length})`;
                }, myFactoriesAtAirport, currentLanguage)}
              </div>
            </Button>
          </div>
          ...
          <div style="margin-bottom: 8px;">
            <Button style="display: block; width: 100%;" callback={(): void => {
              ...
              <div style="padding: 12px 0; background: #22c55e; border-radius: 8px; color: #1a1a24; font-size: 13px; font-weight: 600; text-align: center;">
          ...
          <div style={MappedSubject.create(([airport, userAirport]) => {
            ...
            <Button style="display: block; width: 100%;" callback={(): void => {
              ...
              <div style="padding: 12px 0; background: #f59e0b; border-radius: 8px; color: #1a1a24; font-size: 13px; font-weight: 600; text-align: center;">

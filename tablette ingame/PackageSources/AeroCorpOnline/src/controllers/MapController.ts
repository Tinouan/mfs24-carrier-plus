...
export class MapController {
  ...
  public initializeMapManager(): void {
    mapManager.initialize({
      ...
      onAirportsLoaded: (count) => {
        console.log("[ACO] Airports loaded:", count);
      },
      onFactoriesLoaded: (count) => {
        console.log("[ACO] Factories loaded:", count);
      },
      onHelipadsLoaded: (count) => {
        console.log("[ACO] Helipads loaded:", count);
      },
      onMapMoveEnd: () => {
        ...
        if (anyAirportsVisible && (zoomCrossedThreshold || mapManager.shouldReloadAirports(bounds))) {
          console.log(`[ACO] Map moved - reloading airports (zoom: ${this.lastLoadedZoom.toFixed(1)} → ${currentZoom.toFixed(1)}, threshold: ${zoomCrossedThreshold})`);
  ...
  public initializeMap(): void {
    ...
    if (!container) {
      console.log("[ACO] Map container not found");
      return;
    }
    ...
    if (!simVarValid) {
      ...
      if (user?.last_latitude && user?.last_longitude) {
        ...
        foundFallback = true;
        console.log(`[ACO] Map init: Using Player.last_latitude/longitude (${lat}, ${lon})`);
      }
      ...
      if (!foundFallback) {
        ...
        if (airportIcao) {
          ...
          if (airport && airport.latitude && airport.longitude) {
            ...
            foundFallback = true;
            console.log(`[ACO] Map init: Using player airport ${airportIcao} for marker (${lat}, ${lon})`);
          }
        }
      }

      if (!foundFallback) {
        console.log("[ACO] Map init: No fallback position available, marker will be at 0,0");
      }
    }
    ...
    console.log("[ACO] Map initialized successfully!");
    ...
    if (anyAirportsVisible) {
      console.log("[ACO] Filters active on init, loading airports...");
    ...
    if (mapState.showHelipadsOnMap.get()) {
      console.log("[ACO] Helipad filter active on init, loading helipads...");
  ...
  private async fetchMyFactoriesAtAirport(icaoCode: string): Promise<void> {
    ...
    try {
      const allFactories = await WorldRouter.getFactoriesAtAirport(icaoCode);
      const playerFactories = allFactories.filter((f: any) => f.tier !== 0);
      mapState.myFactoriesAtAirport.set(playerFactories);
    } catch (error) {
      console.error("[ACO] Failed to fetch factories:", error);
      mapState.myFactoriesAtAirport.set([]);
    }
  }
  ...
  private async fetchAvailableSlotsAtAirport(icaoCode: string): Promise<void> {
    try {
      ...
      console.error("[ACO] Failed to fetch slots:", error);
  ...
  public async fetchNearbyAirports(): Promise<void> {
    ...
    console.log("[ACO] Fetching nearby airports for:", lat, lon);
    ...
    try {
      ...
      console.log("[ACO] Nearby airports:", airports);
      ...
      console.error("[ACO] Nearby airports FAILED:", errorMsg);
  ...
  public async centerMapOnPlayerAirport(): Promise<void> {
    ...
    try {
      ...
      if (targetIcao) {
        ...
        if (airport && airport.latitude && airport.longitude) {
          console.log(`[ACO] Centering on player airport: ${targetIcao} (${airport.latitude}, ${airport.longitude})`);
      ...
      if (user?.last_latitude && user?.last_longitude) {
        console.log(`[ACO] Centering on player saved position: (${user.last_latitude}, ${user.last_longitude})`);
      ...
      if (Math.abs(svLat) > 0.1) {
        console.log(`[ACO] Centering on SimVar position: (${svLat}, ${svLon})`);
      ...
      console.log("[ACO] No player position found, defaulting to LFPG");
      this.olMap.getView().animate({
      ...
      console.warn("[ACO] Could not center on player airport:", e);
    }
  }
  ...
  public async searchAirportByIcao(): Promise<void> {
    ...
    try {
      ...
      console.log(`[ACO] Centered on ${icao}: ${airport.name || ""} (${airportLat}, ${airportLon})`);
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      console.error("[ACO] ICAO search failed:", errorMsg);
  ...
  private fetchAirportsForMap(): void {
    ...
    console.log(`[ACO] Zoom ${zoom.toFixed(1)} - Fetched ${airports.length} airports (types: ${types.join(",")})`);
  ...
  private fetchFactoriesForMap(): void {
    ...
    console.log(`[ACO] Loaded ${mapFactories.length} factories on map`);
  }
  ...
  private async fetchHelipadsForMap(): Promise<void> {
    ...
    if (zoom < 10) {
      console.log(`[ACO] Zoom ${zoom.toFixed(1)} too low for helipads`);
    ...
    try {
      ...
      console.log(`[ACO] Zoom ${zoom.toFixed(1)} - Fetched ${helipads.length} helipads`);
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      console.error("[ACO] Fetch helipads for map FAILED:", errorMsg);

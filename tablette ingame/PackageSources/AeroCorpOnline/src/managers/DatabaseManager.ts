...
const STORAGE_PREFIX = "aco_";
const DATASTORE_KEY = "AeroCorpOnlineData";

// Stores that need persistent backup (critical game data)
// ALL game-state stores must be here to survive localStorage clears between MSFS sessions
const PERSISTENT_STORES: StoreName[] = [
  "player",
  "company",
  "company_members",
  "aircraft",
  "aircraft_systems",           // Aircraft engine/propeller/electrical conditions
  "aircraft_damage_log",        // Damage history
  "aircraft_maintenance_log",   // Maintenance records
  "inventory",
  "missions",
  "mission_legs",               // Active mission legs
  "mission_waypoints",          // Active mission waypoints
  "mission_events",             // Mission events
  "mission_flight_data",        // Flight telemetry
  "mission_cargo",              // Cargo per mission
  "pilot_career_stats",
  "free_flight_sessions",
...
class DatabaseManagerClass {
  ...
  async initialize(callbacks: DatabaseCallbacks = {}): Promise<void> {
    ...
    // ── ONE-TIME MIGRATION: WOA → ACO ──
    // Migrate old storage keys to new naming convention
    try {
      // Migrate MSFS DataStore key
      if (typeof GetStoredData === "function" && typeof SetStoredData === "function") {
        const oldData = GetStoredData("WorldOfAircraftData");
        if (oldData && oldData !== "" && !GetStoredData("AeroCorpOnlineData")) {
          SetStoredData("AeroCorpOnlineData", oldData);
          console.log("[DatabaseManager] Migrated WorldOfAircraftData → AeroCorpOnlineData");
        }
      }
      // Migrate localStorage keys: woa_* → aco_*
      const keysToMigrate = Object.keys(localStorage).filter(k => k.startsWith("woa_"));
      for (const oldKey of keysToMigrate) {
        const newKey = "aco_" + oldKey.substring(4);
      ...
      if (keysToMigrate.length > 0) {
        console.log(`[DatabaseManager] Migrated ${keysToMigrate.length} localStorage keys (woa_* → aco_*)`);
      }
      // Migrate sessionStorage key
      try {
        const oldSession = sessionStorage.getItem("WorldOfAircraftData");
        if (oldSession && !sessionStorage.getItem("AeroCorpOnlineData")) {
          sessionStorage.setItem("AeroCorpOnlineData", oldSession);
          console.log("[DatabaseManager] Migrated sessionStorage WorldOfAircraftData → AeroCorpOnlineData");
        }
      } catch { /* sessionStorage may not be available */ }
    } catch (e) {
      console.warn("[DatabaseManager] Migration WOA→ACO failed (non-critical):", e);
    }
    ...
    try {
      ...
      const allKeys = Object.keys(localStorage).filter(k => k.startsWith(STORAGE_PREFIX));
      console.log(`[DatabaseManager] Found ${allKeys.length} aco_ keys in localStorage:`, allKeys);
      ...
      // Start periodic DataStore backup (every 60s) to prevent data loss between sessions
      if (this.autoSaveInterval === null) {
        this.autoSaveInterval = window.setInterval(() => {
          void this.saveToDataStore();
        }, 60000);
        console.log("[DatabaseManager] Periodic DataStore backup started (every 60s)");
      }
  ...
  private readonly BACKUP_FILE = "coui://html_ui/efb_ui/efb_apps/AeroCorpOnline/woa_backup.json";
  ...
  /**
   * Force an immediate save to DataStore, cancelling any pending debounce.
   * Use for critical data (player position) and before app close.
   */
  forceSaveSync(): void {

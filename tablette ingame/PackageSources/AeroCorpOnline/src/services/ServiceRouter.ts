...
export const WorldRouter = {
  ...
  async getFactoriesAtAirport(icao: string): Promise<any[]> {
    return Services.factories.getFactoriesAtAirport(icao);
  },

  async getAvailableSlots(icao: string): Promise<number> {
    const airport = await Services.world.getAirport(icao);
    const airportType = airport?.type || "small_airport";
    return Services.factories.getRemainingSlots(icao, airportType);
  },
...
export const ContractRouter = {
  ...
  async generateAIContracts(playerAirport: string): Promise<void> {
    return Services.contracts.generateAIContracts(playerAirport);
  },
};

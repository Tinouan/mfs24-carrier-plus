...
class NativePersistenceClass {
   ...
   * Solo mode: "ACO_Solo_"
   * Online mode: "ACO_Online_"
   */
  private getModePrefix(mode?: GameMode | null): string {
    const currentMode = mode ?? getCurrentGameMode();
    if (currentMode === "solo") return "ACO_Solo_";
    if (currentMode === "online") return "ACO_Online_";
   ...
   * Key is auto-prefixed with "ACO_" for namespace isolation
   */
  get(key: string): string | null {
    ...
    try {
      let value = GetStoredData(`ACO_${key}`);
      // Migration: check old WOA_ prefix
      if (!value || value.length === 0) {
        const oldValue = GetStoredData(`WOA_${key}`);
        if (oldValue && oldValue.length > 0) {
          SetStoredData(`ACO_${key}`, oldValue); // Migrate
          value = oldValue;
        }
      }
      if (value && value.length > 0) {
  ...
  async load(): Promise<SaveData | null> {
    ...
    try {
      ...
      // Migration: Check old WOA_ mode-prefixed key
      if (!json || json === "") {
        const oldModeKey = storageKey.replace("ACO_", "WOA_");
        const oldJson = GetStoredData(oldModeKey);
        if (oldJson && oldJson !== "") {
          console.log(`[NativePersistence] Migrating save data from ${oldModeKey} to ${storageKey}`);
          SetStoredData(storageKey, oldJson);
          json = oldJson;
        }
      }

      // Migration: Check legacy non-prefixed key WOA_SaveData
   ...
   *
   * FIX: Now checks "AeroCorpOnlineData" (where DatabaseManager actually saves)
  ...
  isSetupComplete(mode?: GameMode | null): boolean {
    ...
    let json = GetStoredData(DATASTORE_KEY);

    // Migration: check old key if new one is empty
    if (!json || json === "") {
      json = GetStoredData("WorldOfAircraftData");
    }
    ...
    try {
      ...
      if (!hasPlayer) {
        console.log(`[NativePersistence] No player found in DataStore`);
        return false;
      }
      console.log(`[NativePersistence] Setup complete - found player in DataStore`);
      return true;
    } catch (e) {
      console.warn(`[NativePersistence] Failed to parse DataStore:`, e);
      return false;
    }
  }
  ...
  getPlayerId(mode?: GameMode | null): string | null {
    ...
    const localStorageKey = `aco_${(mode ?? getCurrentGameMode()) || ""}_player_id`;
    // Migration: check old woa_ localStorage key
    const oldLocalStorageKey = `woa_${(mode ?? getCurrentGameMode()) || ""}_player_id`;

    if (!this.isAvailable()) {
      // Fallback to localStorage if native APIs not available
      return localStorage.getItem(localStorageKey) || localStorage.getItem(oldLocalStorageKey);
    }
    ...
    try {
      ...
      const localId = localStorage.getItem(localStorageKey) || localStorage.getItem(oldLocalStorageKey);
      if (localId) {

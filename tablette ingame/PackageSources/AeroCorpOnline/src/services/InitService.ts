...
class InitServiceClass {
  ...
  async initialize(callbacks: InitCallbacks = {}): Promise<void> {
    ...
    try {
      ...
      await DatabaseManager.initialize({
        ...
        // which saves to "AeroCorpOnlineData" key. No need for separate NativePersistence callback.
      });
   ...
   * Called from AeroCorpOnline when user selects a mode
   */
  ...
  private async initializeSoloMode(): Promise<void> {
    ...
    try {
      const rawSetupNew = typeof GetStoredData === "function" ? GetStoredData("ACO_Solo_SetupComplete") : "N/A";
      const rawSetupOld = typeof GetStoredData === "function" ? GetStoredData("WOA_SoloSetupComplete") : "N/A";
      const rawSaveData = typeof GetStoredData === "function" ? GetStoredData("ACO_Solo_SaveData") : "N/A";
      const rawPlayerId = typeof GetStoredData === "function" ? GetStoredData("ACO_Solo_PlayerId") : "N/A";
    ...
    // which loads from "AeroCorpOnlineData" key. No need for separate NativePersistence restore.
    ...
    // which saves to "AeroCorpOnlineData". No need for separate NativePersistence auto-save.
  ...
  private async loadAirportsMain(): Promise<void> {
    ...
    try {
      ...
      const response = await fetch("coui://html_ui/efb_ui/efb_apps/AeroCorpOnline/airports-main.json");
      if (response.ok) {
  ...
  async completeFirstLaunch(
    ...
    // This eliminates ghost data that was restored from AeroCorpOnlineData
    ...
    // 8. CRITICAL: Force save new player data to AeroCorpOnlineData backup
    // This ensures the NEW player (100,000 CR) overwrites any ghost data
    await DatabaseManager.forceSave();
    console.log("[InitService] New player data saved to AeroCorpOnlineData backup");
  ...
  async resetDatabase(): Promise<void> {
    ...
    // V4.1 FIX: Clear DatabaseManager's own backup (DATASTORE_KEY = "AeroCorpOnlineData")
    // This is a SEPARATE persistence system from NativePersistence!
    try {
      if (typeof SetStoredData === "function") {
        SetStoredData("AeroCorpOnlineData", "");
        // Also clear old key
        SetStoredData("WorldOfAircraftData", "");
        console.log("[InitService] Cleared AeroCorpOnlineData + WorldOfAircraftData from SetStoredData");
      }
    } catch (e) {
      console.warn("[InitService] Failed to clear AeroCorpOnlineData:", e);
    }

    // Also clear sessionStorage backup
    try {
      sessionStorage.removeItem("AeroCorpOnlineData");
      sessionStorage.removeItem("WorldOfAircraftData");
      console.log("[InitService] Cleared AeroCorpOnlineData from sessionStorage");
    } catch (e) {
    ...
    try {
      // Clear all ACO/WOA-related localStorage keys
      const keysToRemove: string[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key && (key.startsWith("ACO_") || key.startsWith("aco_") || key.startsWith("WOA_") || key.startsWith("woa_") || key.startsWith("carrier_"))) {
   ...
   * Uses DatabaseManager.forceSave() which saves to "AeroCorpOnlineData" key
   */

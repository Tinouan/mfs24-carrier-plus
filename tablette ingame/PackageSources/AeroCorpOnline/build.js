...
// Chemin de déploiement Community2024
const DEPLOY_PATHS = {
  community: path.join(process.env.LOCALAPPDATA, "Packages", "Microsoft.Limitless_8wekyb3d8bbwe", "LocalCache", "Packages", "Community2024", "aerocorp-online-efb", "html_ui", "efb_ui", "efb_apps", "AeroCorpOnline"),
};
...
const deployPlugin = {
  ...
  setup(build) {
    build.onEnd((result) => {
      ...
      const filesToCopy = ["AeroCorpOnline.js", "AeroCorpOnline.css", "AeroCorpOnline.js.map", "AeroCorpOnline.css.map", "airports-main.json"];
      ...
      for (const [name, destPath] of Object.entries(DEPLOY_PATHS)) {
        try {
          ...
          // Copie layout.json + manifest.json au root du package
          const packageRoot = path.resolve(destPath, "..", "..", "..", "..", "..");
          if (fs.existsSync(packageRoot)) {
            const layoutSrc = path.join(__dirname, "..", "..", "layout.json");
            const manifestSrc = path.join(__dirname, "..", "..", "manifest.json");
            if (fs.existsSync(layoutSrc)) fs.copyFileSync(layoutSrc, path.join(packageRoot, "layout.json"));
            if (fs.existsSync(manifestSrc)) fs.copyFileSync(manifestSrc, path.join(packageRoot, "manifest.json"));
          }
...
const baseConfig = {
  entryPoints: ["src/AeroCorpOnline.tsx"],
  keepNames: true,
  bundle: true,
  outdir: "dist",
  ...
  target: "es2017",
  define: { BASE_URL: `"coui://html_ui/efb_ui/efb_apps/AeroCorpOnline"` },
  plugins: [
    svgTextPlugin,
    copyStaticFiles({
      src: "./src/Assets",
